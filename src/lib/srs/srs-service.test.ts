...
describe("SrsService", () => {
  ...
  it("advances Lv1 to Lv2 and schedules the first SRS review in 1 day", () => {
    const result = SrsService.processLearningResult({
      mode: "dictation",
      answerResult: "correct",
  ...
  it("requires due review to advance from Lv2 to Lv3", () => {
    const blocked = SrsService.processLearningResult({
      mode: "learn",
      answerResult: "correct",
      currentState: { level: 2, progress: 0, nextReviewAt: "2026-07-10T00:00:00.000Z", intervalDays: 1 },
      now,
    });
    ...
    const result = SrsService.processLearningResult({
      mode: "learn",
      answerResult: "correct",
      currentState: { level: 2, progress: 0, nextReviewAt: "2026-07-08T00:00:00.000Z", intervalDays: 1 },
      now,
    });
  ...
  it("does not change level or scheduling for early correct review answers", () => {
    const result = SrsService.processLearningResult({
      mode: "review",
      answerResult: "correct",
      currentState: { level: 2, progress: 0, nextReviewAt: "2026-07-09T00:00:00.000Z", intervalDays: 1 },
      now,
    });
